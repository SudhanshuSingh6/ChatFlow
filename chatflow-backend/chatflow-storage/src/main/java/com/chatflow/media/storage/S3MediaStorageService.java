...
/**
 * S3 / MinIO storage. Same code for both: MinIO is S3-compatible and differs only by endpoint URL
 * (configured in {@link S3Config}). The bucket is private — {@link #getUrl} returns the object URL
 * for record-keeping, but objects are only retrievable through a {@link #presignedUrl}. Active
 * under the {@code s3} profile.
 */
@Slf4j
@Component
@Profile("s3")
...
public class S3MediaStorageService implements MediaStorageService {
    ...
    public void delete(String storageKey) {
        try {
            ...
            // must surface so the cleanup job retries.
