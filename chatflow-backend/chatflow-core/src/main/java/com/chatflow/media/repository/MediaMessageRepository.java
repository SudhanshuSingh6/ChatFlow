...
public interface MediaMessageRepository extends JpaRepository<MediaMessage, UUID> {
    ...
    Optional<MediaMessage> findByMessageIdAndDeletedFalse(UUID messageId);
    ...
    /**
     * Rows marked for deletion whose storage purge has not completed (Phase 7
     * retry). The cutoff excludes freshly-marked rows so the scheduled job does
     * not race the post-commit purge.
     */
    @Query("SELECT m FROM MediaMessage m WHERE m.status = 'PENDING_DELETION' " +
            "AND m.updatedAt < :cutoff")
    List<MediaMessage> findPendingDeletions(
            @Param("cutoff") java.time.LocalDateTime cutoff);
}